...
get '/profile' do
	...
	follow = Follow.all(user_id1: session[:user_id])
	follow1 = Follow.all(user_id1: session[:user_id]).first

	erb :profile, locals: {user: user, tweet: tweet, user1: user1, follow: follow, follow1: follow1}
end
